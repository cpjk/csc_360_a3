...
int main(int argc, char **argv) {
  ...
  if (close (fd) == -1) { perror ("close"); return 1; } // close the file descriptor

  fd = open(argv[2], O_RDONLY); // get a file descriptor for the input file
  if(fd == -1) { perror("open"); return 1; }
  if(fstat(fd, &sb2) == -1) { perror("fstat"); return 1; }
  input_file = (char *) mmap(0, sb2.st_size, PROT_READ, MAP_SHARED, fd, 0); // mmap the disk to memory
  if (disk == MAP_FAILED) { perror ("mmap"); return 1; }
